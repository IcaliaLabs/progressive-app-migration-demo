...
# 4: Install the development & runtime packages:
RUN set -ex && \
 apk add --no-cache build-base bash ca-certificates curl openssl tzdata su-exec
...
RUN export YARN_VERSION=0.24.6 \
...
RUN npm install --global check-dependencies
...
ENV HOME=/usr/src/app PATH=/usr/src/app/bin:$PATH PAGER='busybox less'

# 5: Copy just the Gemfile & Gemfile.lock, to avoid the build cache failing whenever any other
# file changed and installing dependencies all over again - a must if your'e developing this
# Dockerfile...
ADD ./Gemfile* /usr/src/app/

# 6: Install the app gems:
RUN set -ex && bundle install --jobs=4 --retry=3

# 7: Expose the app web port:
EXPOSE 3000
